...
class EcorFileHandler(tsdat.AbstractFileHandler):
    """--------------------------------------------------------------------------------
    ...
    def read(self, filename: str, **kwargs) -> xr.Dataset:
        ...
        for i in range(len(variables)):
            data = np.array(list(map(float, content[40 + i].split()[1:-1])))
            cov[i, : len(data)] = data

        cov_rot = np.empty((7, 7))
        cov_rot[:] = np.nan
        for i in range(len(variables)):
            data = np.array(list(map(float, content[50 + i].split()[1:-1])))
        ...
        ds["covariance"] = xr.DataArray(
            cov, coords={"var": variables, "var*": [v + "*" for v in variables]}
        )
        ds["covariance_rotated"] = xr.DataArray(
            cov_rot, coords={"var": variables, "var*": [v + "*" for v in variables]}
        )
