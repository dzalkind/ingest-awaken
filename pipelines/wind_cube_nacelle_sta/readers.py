...
class WindCubeReaderSTA(DataReader):
    """---------------------------------------------------------------------------------
    ...
    def read(self, input_key: str) -> Union[xr.Dataset, Dict[str, xr.Dataset]]:
        ...
        for var in dataset.data_vars:
            if var not in output_var_names:
                var_data = np.reshape(
                    dataset[var].values,
                    (len(dataset[var]) // len(distances), len(distances)),
                )
                dataset[var] = xr.DataArray(var_data, dims=["time", "distance"])
